...
CREATE TABLE `Resource` (
    ...
    `title` ENUM('WHEAT_SEED', 'WHEAT', 'WHEAT_SACK', 'WHEAT_SACK_PALLETE_PRE', 'WHEAT_SACK_PALLETE_POST', 'FLOUR', 'BREAD_PRODUCT', 'DISTRIBUTED_BREAD_PRODUCT', 'BREAD_PRODUCT_CONSUMER') NOT NULL DEFAULT 'DISTRIBUTED_BREAD_PRODUCT',
...
CREATE TABLE `TraceabilityToken` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `orderId` INTEGER NOT NULL,
    `bcTokenId` INTEGER NOT NULL,
    `token` TEXT NOT NULL,

    UNIQUE INDEX `TraceabilityToken_orderId_key`(`orderId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Change Autostart starting value
ALTER TABLE `Customer` AUTO_INCREMENT=8001;
