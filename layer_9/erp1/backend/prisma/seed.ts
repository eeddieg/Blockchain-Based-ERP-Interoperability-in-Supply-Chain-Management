...
const Customers: Prisma.CustomerCreateInput[] = [
  {
    name: "L9CA_Customer 1",
    email: "l9ca_cust1@test.com",
  },
  // {
  //   name: "Customer 9",
  ...
  for (const cust of Customers) {
    ...
    const resourcesCreated = await db.resource.create({
      data: {
        ...
        title: Product.BREAD_PRODUCT_CONSUMER,
        amount: 1000,
      }
    });
...
async function addToCustomer(id: number, amount: number, type: OrderType) {
  ...
  console.log(`New order created from ${amount} untis of ${Product.BREAD_PRODUCT_CONSUMER}`);
