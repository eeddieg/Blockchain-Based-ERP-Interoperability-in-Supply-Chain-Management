...
export default class TokenStoreModule extends VuexModule {
  ...
  private elementalPedigreeObject = null;
  private compositePedigreeObject = null;
  ...
  get elementalPedigree() {
    return this.elementalPedigreeObject;
  }
  get compositePedigree() {
    return this.compositePedigreeObject;
  }
  get processes() {
  ...
  @Mutation
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  [Mutations.SET_COMPOSITE_PEDIGREE_TOKEN](payload: any) {
    this.compositePedigreeObject = { ...payload };
  }
  @Mutation
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  [Mutations.SET_ELEMENTAL_PEDIGREE_TOKEN](payload: any) {
    this.elementalPedigreeObject = { ...payload };
  }
  @Mutation
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  @Mutation
  [Mutations.UNSET_COMPOSITE_PEDIGREE_TOKEN]() {
    this.compositePedigreeObject = null;
  }
  @Mutation
  [Mutations.UNSET_ELEMENTAL_PEDIGREE_TOKEN]() {
    this.elementalPedigreeObject = null;
  }
  @Mutation
  ...
  @Action
  async [Actions.GET_PEDIGREE_BY_ID_FROM_BC](id: number) {
    await this.context.commit("clearElementalPedigreeToken");

    const url =
      AxiosBc.defaults.baseURL + "/token" + "/mutable" + "/pedigree" + "/" + id;
    const token = localStorage.getItem("token") || null;
    if (token != null && token !== "") {
      AxiosBc.defaults.headers.post["Content-Type"] = "application/json";
      AxiosBc.defaults.headers.post.Authorization = "Bearer " + token;
    ...
    try {
      ...
      const pedigreeToken = res.data.pedigree;

      //   {
      //     "id": 1,
      //     "pedigreeIdToken": 1,
      //     "quantity": 821,
      //     "timestamp": "1726319916028",
      //     "maker": "0x1Fd644b3b7f98be0D608525AA08e58020AD84DfE",
      //     "hashIPFS": "0xf62219a530e7139d54622521811a96caa126302faa9556727955e59958e3b834"
      // }

      if (pedigreeToken != null) {
        this.context.commit("setElementalPedigreeToken", pedigreeToken);
  ...
  async [Actions.GET_UPDATED_TRACE_TOKEN_ID_FROM_BC]() {
    ...
    try {
      ...
      const previousLayer = (layerBase + newLayer) as string;
      ...
      console.log("Actions.GET_UPDATED_TRACE_TOKEN_ID_FROM_BC - ERROR");
      console.log(error);
    }
  }
  @Action
  ...
  @Action
  async [Actions.REMOVE_UPDATED_TOKEN_FILE_FROM_BC]() {
    const url =
      AxiosBc.defaults.baseURL +
      "/token" +
      "/mutable" +
      "/token/removeUpdatedToken";
    ...
    try {
      ...
      console.log("Actions.REMOVE_UPDATED_TOKEN_FILE_FROM_BC - ERROR");
      console.log(error);
    }
  }
  @Action
  ...
  @Action
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async [Actions.UPDATE_TRACE_TOKEN_INFO_ON_BC](data: any) {
    const url =
      AxiosBc.defaults.baseURL + "/token" + "/mutable" + "/token/update/token";
    const token = localStorage.getItem("token") || null;
    if (token != null && token !== "") {
      AxiosBc.defaults.headers.post["Content-Type"] = "application/json";
      AxiosBc.defaults.headers.post.Authorization = "Bearer " + token;
    ...
    try {
      ...
      console.log("Actions.UPDATE_TRACE_TOKEN_INFO_ON_BC - ERROR");
      console.log(error);
    }
  }
  @Action
