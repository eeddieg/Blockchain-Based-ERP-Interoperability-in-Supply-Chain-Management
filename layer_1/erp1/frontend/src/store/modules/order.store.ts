...
export default class OrderStoreModule extends VuexModule {
  ...
  async [Actions.CREATE_ORDER](data: any) {
    ...
    try {
      ...
      if (createdOrderRes.data !== null) {
        ...
        const serializedToken = await this.context.dispatch(
          "generateTraceabilityToken",
          id
        );
        ...
        const regOrderBcPayload = {
          customer,
          order: { serializedToken, ...createdOrder },
        };
  ...
  @Action
  private [Actions.GENERATE_TRACEABILITY_TOKEN](id: number) {
    const company = this.context.getters.getCompany as Company;
    const productToBuy = company.productToBuy as ProductBc;

    const stakeholderToken = company.name;
    const productionToken = productToBuy + ".orderId_" + id;
    const processToken = "";
  ...
  @Action
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async [Actions.STORE_ORDER_TO_DB](data: any) {
    const storeOrderUrl = Axios.defaults.baseURL + "/order" + "/storeOrder";
    const token = localStorage.getItem("token") || null;
    if (token != null && token !== "") {
      Axios.defaults.headers.post["Content-Type"] = "application/json";
      Axios.defaults.headers.post.Authorization = "Bearer " + token;
    ...
    try {
      const res = await Axios.post(storeOrderUrl, orderPayload);
  ...
  @Action
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async [Actions.STORE_RESOURCE_TO_DB](data: any) {
    const storeOrderUrl = Axios.defaults.baseURL + "/order" + "/storeResources";
    const token = localStorage.getItem("token") || null;
    if (token != null && token !== "") {
      Axios.defaults.headers.post["Content-Type"] = "application/json";
      Axios.defaults.headers.post.Authorization = "Bearer " + token;
    ...
    try {
      const res = await Axios.post(storeOrderUrl, resourcesData);
