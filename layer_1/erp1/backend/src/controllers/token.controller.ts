...
class TokenController {
  ...
  // TraceToken
  static async storeTraceToken(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const payload = {
        orderId: req.body.orderId as number,
        bcTokenId: req.body.bcTokenId as number,
        token: req.body.token as string,
      } as TraceabilityToken;

      const storedToken = await TokenService.storeTraceToken(payload) as TraceabilityToken;

      res.status(200).json({
        status: true,
        statusCode: 200,
        message: "Traceability token stored successfully.",
        token: storedToken,
      });

    } catch (error: any) {
      console.log("\nTokenController.storeTraceToken - ERROR");
      res.status(500).json({
        status: false,
        statusCode: 500,
  ...
  static async triggerUpdatedTokenEvent(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      res.status(200).json({
        status: false,
        statusCode: 500,
        message: "TokenController.updateOrder - ERROR",
        error,
      });
    }
  }
  
  static async updateOrderWithTraceTokenId(req: Request, res: Response, next: NextFunction): Promise<void> {
